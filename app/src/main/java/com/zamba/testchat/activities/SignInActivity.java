...
import com.zamba.testchat.models.Country;
import com.zamba.testchat.models.User;
import com.zamba.testchat.services.SinchService;
import com.zamba.testchat.utils.Helper;
import com.zamba.testchat.utils.KeyboardUtil;
...
public class SignInActivity  extends AppCompatActivity {
    ...
    private DatabaseReference userRef, newUserRef,adminRef;
...
//            else {
        ...
        PhoneAuthProvider.getInstance().verifyPhoneNumber(phone, 60, TimeUnit.SECONDS, this,
                new PhoneAuthProvider.OnVerificationStateChangedCallbacks() {
                    ...
                    public void onVerificationCompleted(PhoneAuthCredential phoneAuthCredential) {
                        ...
                        Log.e( TAG, "onVerificationCompleted: " + phoneAuthCredential.getProvider());
                        Log.e( TAG, "onVerificationCompleted: " + phoneAuthCredential.getSmsCode());
                        Log.e( TAG, "onVerificationCompleted: " + phoneAuthCredential.getSignInMethod());
                    }
    ...
    private void login() {
        authInProgress = true;
        Log.e(TAG,"login  call");

        Log.e(" phoneNumberInPrefs " ,phoneNumberInPrefs);

        SharedHelper.putKey(getApplicationContext(),"phone_no",phoneNumberInPrefs);
        ...
        userRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                if (dataSnapshot.exists() && dataSnapshot.getChildrenCount() > 0) {
                    try {
                                ...
                                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                    ...
                                    while (items.hasNext()) {
                                        try {
                                            ...
                                            String phonen = String.valueOf(data.child("id").getValue());


                                            Log.e(TAG," admin  "+ "phoneNumber : " + phoneNumber);


                                            phoneNumber=SharedHelper.getKey(getApplicationContext(),"phone_no");
                                            Log.e(TAG,"  admin  "+ "phoneNumber : " + phoneNumber);
                                            Log.e( TAG," admin  "+ "phoneNumber : " + phonen);

                                            if (phonen.equalsIgnoreCase(phoneNumber))
                                            {
                                                Log.e( TAG," admin  "+ "login : " + phonen);
                                                SharedHelper.putKey(getApplicationContext(),"login_as","admin");
                                        ...
                                        }catch (Exception ee)
                                        {
                                            ...
                                            Log.e(TAG," admin "+"Exception "+ee.getMessage());
                                        }
                                    }
                                }
                        ...
                        Log.e(TAG,"user "+"Exception "+ex.getMessage());
                        createUser(new User(phoneNumberInPrefs, phoneNumberInPrefs, getString(R.string.app_name), "" ));
                    }
                } else {
                    Query query = FirebaseDatabase.getInstance().getReference().child("admin");
                        ...
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            ...
                            while ((snapshotIterator.hasNext())) {
                                ...
                                try {
                                    ...
                                    phoneNumber=SharedHelper.getKey(getApplicationContext(),"phone_no");
                                    Log.e(TAG,"  ADMIN  "+ "phoneNumber : " + phoneNumber);
                                    Log.e(TAG,"  db  "+ "phoneNumber : " + phonen);
            ...
            public void onCancelled(DatabaseError databaseError) {
                ...
                Log.e(TAG,"user  "+ "databaseError : " + databaseError);
...
//        return toReturn;
    ...
    private void register(User userMe) {
        userMe.setNameInPhone("You");
        User userMeInDb = rChatDb.where(User.class).equalTo("id", userMe.getId()).findFirst();
        if (userMeInDb == null) {
            rChatDb.beginTransaction();
    ...
    private void doneAdmin(User userMe) {
        userMe.setNameInPhone("You");
        User userMeInDb = rChatDb.where(User.class).equalTo("id", userMe.getId()).findFirst();
        if (userMeInDb == null) {
            rChatDb.beginTransaction();
    ...
    public void submit() {
        ...
        String  myphone=etPhone.getText().toString().replaceAll("\\s+", "");


        Log.e("phone_no  ","before  "+myphone);
        char  my_char=0;
        myphone= removeLeadingChar(myphone,my_char);

        if (myphone.startsWith("0"))
        {
            Log.e("phone_no  ","startsWith 0  "+myphone);
        ...
        Log.e("phone_no  ","after  "+myphone);
        phoneNumber = ((Country) spinnerCountryCodes.getSelectedItem()).getDialCode() + myphone;

        Log.e("phone_no  ","phoneNumber  "+phoneNumber);
        if (!Patterns.PHONE.matcher(phoneNumber).matches()) {
            Toast.makeText(this, R.string.validation_req_phone_valid, Toast.LENGTH_LONG).show();
            return;
        }
        SharedHelper.putKey(getApplicationContext(),"phone_no",phoneNumber);
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
    ...
    public  String removeLeadingChar(String str, char ch) {
        int idx = 0;
